...
export class GitTogetherMatchesComponent {
  ...
  deleteMatch(pid: number) {
    this.matchService
      .deleteMatch(this.profile.pid, pid, this.selectedCourse)
      .subscribe({
        next: (data: any) => {
  ...
  getMatches(courseCode: string) {
    try {
      this.matchService.get_matches(courseCode, this.profile.pid).subscribe({
        next: (data) => {
          if (data.length > 0) {
            this.matches = [];
  ...
  getNewMatches(courseCode: string) {
    try {
      this.matchService
        .get_new_matches(courseCode, this.profile.pid)
        .subscribe({
          next: (data) => {
