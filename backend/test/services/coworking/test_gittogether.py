...
def test_get_matches_without_initial_form(gt_svc: GitTogetherService, session: SessionDI):
    ...
    with pytest.raises(InitialFormError, match="Fill out initial form first"):
        gt_svc.get_matches("COMP110", 1, mock_openai, session, mock_usersvc)

def test_get_matches_without_specific_form(gt_svc: GitTogetherService, session: SessionDI):
...
def test_delete_specific_answer(gt_svc: GitTogetherService, session: SessionDI):
    ...
    # Verify it exists
    assert session.query(SpecificFormEntity).filter_by(pid=1, clas="COMP110").first() is not None
    
    # Delete it
    gt_svc.delete_student_specifc_answer(1, "COMP110", session)
    
    # Verify it's gone
    assert session.query(SpecificFormEntity).filter_by(pid=1, clas="COMP110").first() is None
...
def test_get_teacher_pairings_list_empty(gt_svc: GitTogetherService, session: SessionDI):
...
def test_get_teacher_pairings_list_with_new_students(gt_svc: GitTogetherService, session: SessionDI):
...
def test_get_teacher_pairings_list_odd_number(gt_svc: GitTogetherService, session: SessionDI):
    ...
    gt_svc.class_specific_form(
        FormResponse(
            value="Answer3", pid=3, contact_info="test3@email.com", 
            clas="COMP110", first_name="Student3"
        ),
        session
    )
...
def test_get_stored_teacher_pairings(gt_svc: GitTogetherService, session: SessionDI):
    ...
    session.add_all([
        ...
        TeacherMatchEntity(pid_one=5, pid_two=-1, course="COMP110"),  # Odd student out
        TeacherMatchEntity(pid_one=6, pid_two=7, course="COMP210")   # Different course
    ])
    session.commit()
